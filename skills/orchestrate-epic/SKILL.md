...
3. **Run `CHECKS_SET`'s `runnable` entries once**, from Step 3, against the committed worktree — the Publisher-side check run this loop is required to have. A red result — any entry exiting non-zero — actually blocks sub-steps 4–5, not merely a comment noting that it should: restore the worktree to exactly the worker's `HEAD_SHA` first (`git reset --hard` plus `git clean -fd`, clearing any build output, cache, or partial lockfile the check run left behind, since Step 4 reuses this same worktree on the next dispatch and Step 6 would otherwise see that leftover state as if it were a new change), then handle it exactly like a secret-screen hit in sub-step 1 above — post the failing command and its output as a comment on the Issue, upsert the state comment's `unresolved_blockers`, leave `loop:in-progress` in place, and move on to the next Issue in the round. Nothing is staged or committed after this sub-step on a green result, so a check that writes a lockfile, build output, or cache directory into the worktree never rides into the pushed commit.
