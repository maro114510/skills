...
- **Retry** — a human decision restarts the fix-cycle budget, so reset `cycle` to 0 in the state comment and reclassify as `in-progress` so Step 4 dispatches it fresh this round, with the full comment, already verified as trusted above, included in the worker prompt for whatever context beyond the bare command it carries.
- **Redo on a fresh branch** — handle exactly like the merged-PR-but-issue-open sanity check below: a new branch such as `feat/issue-<number>-2`, fresh worktree, `cycle` reset to 0.
...
- When branch protection has no required checks and no required reviewers: state explicitly that this loop's own reviewer pass in Step 6 is the only merge gate the repository enforces.
...
3. Push and create the PR with `gh pr create --head <branch>` — body carries the worker summary, check evidence from `CHECKS`/`CRITERIA`, any reviewer nits, `Closes #<number>`, and the Epic reference.
...
- The six #141 metrics, best-effort from data this run already has: dispatch counts, state-comment `cycle` values, reviewer first-pass verdicts, round timestamps. Where a metric depends on work not yet done, since CI-failure attribution needs #143's derived check set, say so plainly instead of reporting a number.
