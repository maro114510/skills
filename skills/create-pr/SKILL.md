...
**5.3・5.4 の確認が完了してから Step 6 に進む**（ユーザーの応答を待ってから `gh pr create` を実行する）。
ただし 5.2（ラベル）は AI が自動で決定するため、ユーザーの応答は不要。
...
実行結果と Step 2 の変更分析（種類・影響範囲）を照合し、最も適切なラベルを AI が自動で選択する。
選択したラベルと選択理由を 1 行でユーザーに通知する（確認は不要）。
一致するラベルが存在しない場合はラベルなしで進む。
