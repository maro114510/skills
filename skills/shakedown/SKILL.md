...
1. **接地(anchor)必須**: すべてのシナリオは diff のハンク、コードの file:line、または設計ドキュメントのセクションを根拠として引用する。LLM 生成テストの最大の失敗要因は「仕様に存在しない挙動の発明」であり、anchor はその防壁である。接地できないが重要な横断リスク(キャッシュ不整合、下流消費者への影響など)は削除せず `[unanchored]` タグを付けて合意ゲートで明示確認する。UI 要素(ボタン名・ラベル・遷移先)はソースに実在する文字列だけを引用する。見ていない要素の名前を発明しない。
...
| 引数なし | `git diff origin/main...HEAD`(origin/main がなければ `git branch -r` でデフォルトブランチを特定) |
| `ticket <ID>` | 保存ファイル名と関連ドキュメント探索に使う |

- diff が空なら停止し、対象の指定を求める。
...
```
S<n> [タグ] <シナリオ名> — <1行根拠> (anchor: <file:line | docセクション | [unanchored]>)
```

タグは 3 種:

- `[Blocking]` — 失敗したらリリースを止める。最大 7 件。
- `[Should-verify]` — 確認すべきだが単独ではリリースを止めない。
...
- 1 シナリオ = 1 ゴール・1 フロー。
- 不可逆な副作用(本番データ書き換え、外部 API での発行・課金、通知送信)を持つものは自動的に Blocking。
- 第 1 層の分岐ノードとシナリオ ID を相互参照させる。

## Step 5: anchor 検証(Blocking のみ)

合意ゲートに出す前に、Blocking シナリオの捏造を独立した目で潰す。Agent ツールで読み取り専用のサブエージェントを 1 体起動し、対象リポジトリのパス・基準 SHA・Blocking シナリオの一覧を渡して次を依頼する:
...
REFUTED が返ったシナリオは、修正して再アンカーするか `[unanchored]` に降格して合意ゲートで明示確認する。黙って握り潰さない。
